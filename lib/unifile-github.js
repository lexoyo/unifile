...
class GitHubConnector {
   ...
   */
	[transformTree](session, repo, transformer, message, branch) {
		const targetedBranch = branch || 'master';
		let lastCommitSha;
		const apiPath = '/repos/' + session.account.login + '/' + repo;
		return this[callAPI](session, apiPath + '/git/refs/heads/' + targetedBranch, null, 'GET')
		.then((head) => {
		...
		.then((res) => {
			...
			return this[callAPI](session, apiPath + '/git/refs/heads/' + targetedBranch, data, 'PATCH');
		});
	}
